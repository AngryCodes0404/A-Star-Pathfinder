...
cells = (
    {}
)  # Dictionary of Cells where a tuple (immutable set) of (x,y) coordinates is used as keys

for x in range(num_cells):
    for y in range(num_cells):
        cells[(x, y)] = {
            "state": None,  # None, Wall, Goal, Start Are the possible states. None is walkable
            ...
            "parent": None,
        }  # In order to walk the found path, keep track of how we arrived to each cell
...
heuristic = (
    "crow"  # Could be 'manhattan' or 'crow' anything else is assumed to be 'zero'
)
...
def orthoganals(current):
    ...
    return [
        x
        for x in directions
        if onBoard(x) and cells[x]["state"] != "Wall" and not x in closed_list
    ]
...
def diagonals(current):
    ...
    return [
        ...
        for x in directions
        if onBoard(x)
        and cells[x]["state"] != "Wall"
        and not x in closed_list
        and not blocked_diagnol(current, x)
    ]
    ...
    for event in pygame.event.get():
        ...
        if (x_index, y_index) in cells:
            ...
            if (
                ctrl
                and left_click
                and not start_placed
                and cells[(x_index, y_index)]["state"] != "Start"
                and cells[(x_index, y_index)]["state"] != "Goal"
            ):
                start_placed = True
            ...
            # Place Wall
                ...
                shift
                and left_click
                and cells[(x_index, y_index)]["state"] == None
                and last_wall != (x_index, y_index)
            ):
            ...
            # Remove Wall
                ...
                shift
                and left_click
                and cells[(x_index, y_index)]["state"] == "Wall"
                and last_wall != (x_index, y_index)
            ):
                cells[(x_index, y_index)]["state"] = None
                r = pygame.Rect(left, top, cell_size - 2, cell_size - 2)
            ...
            # Soft Reset Board (keep start, goal and walls)
                ...
                for cell in cells:
                    if (
                        cells[cell]["state"] != "Wall"
                        and cells[cell]["state"] != "Start"
                        and cells[cell]["state"] != "Goal"
                    ):
                        i += 1
                        for x in cells[cell]:
                            cells[cell][x] = None
                        ...
                        cells[cell]["state"] == "Start"
                        or cells[cell]["state"] == "Goal"
                    ):
                        for x in cells[cell]:
                            if x != "state":
                                cells[cell][x] = None
